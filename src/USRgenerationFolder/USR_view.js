...
const USRV = () => {
    ...
    // axios.get(`http://localhost:9999/api/specific_discourse_usr?disco_id=${discourseid}`)
    //     .then(response => {
    ...
    const sendata = () => {
        fetch(`/specific_discourse_usr?disco_id=${discourseid}`)
            .then(response => {
            ...
            .then(datasen => {
                ...
                const r_status = result[index].USR_status;
                setUsrid(result[index].USR_ID);
                const usr_json = JSON.parse(orobj);

                setUsers(data);
                // setSelectedData({ 'Concept': ['eka_1', 'Sera_1', 'jaMgala_1', 'so_1-0_rahA_WA_1'], 'Index': [1, 2, 3, 4], 'SemCateOfNouns': ['', '', '', ''], 'GNP': ['', '[m sg a]', '[m sg a]', ''], 'DepRel': ['2:card', '4:k1', '4:k7p', '0:main'], 'Discourse': ['', '', '', ''], 'SpeakersView': ['', '', '', ''], 'Scope': ['', '', '', ''], 'SentenceType': ['affirmative'] });
                setSelectedData(usr_json)
    ...
    const fetchData = async () => {
        try {
            const response = await fetch("/api/orignal_usr_fetch");
    ...
    // const fetchData = () => {
    //     // fetch(`/api/orignal_usr_fetch3?disco_id=${discourseid}`)
    //     fetch(`/api/specific_discourse_usr`)
    //         .then(response => {
    //             return response.json()
    //         })
    //         .then(data => {
    ...
    //             finalJson = result
    //             const orobj = result[index].orignal_USR_json.replaceAll("\'", "\"")
    //             r_status = result[index].USR_status
    //             setUsrid(result[index].USR_ID);
    //             console.log(usrid)
    //             // console.log(orobj)
    ...
    useEffect(() => {
        ...
        setDiscourseId(searchParams.get('disco_id'))
